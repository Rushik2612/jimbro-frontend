...
const Assessment = () => {
  ...
  const handleDietPrefSubmit = async () => {
    ...
    try {
      ...
      setApiError(`Plan generation failed: ${msg}. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderDietPreferences = () => {
    ...
    return (
      <div className="card" style={{ maxWidth: '600px', margin: '40px auto', padding: '40px' }}>
        ...
        {apiError && (
          <div style={{ marginTop: '12px', padding: '12px 16px', background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.3)', borderRadius: 'var(--radius-md)', color: '#ff6b6b', fontSize: '0.85rem' }}>
            ⚠️ {apiError}
          </div>
        )}
